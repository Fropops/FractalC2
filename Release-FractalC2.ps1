function Update-ProjectVersion {
    ...
    Write-Host "?? Mise � jour de la version pour le projet : $ProjectPath"
    ...
    if ($updatedCsproj) {
        ...
        Write-Host "?? Version mise � jour dans le .csproj"
    }
    ...
    if (-not $assemblyInfo) {
        Write-Warning "?? Aucun fichier AssemblyInfo.cs trouv�."
        return
    }

    Write-Host "?? Fichier AssemblyInfo d�tect� : $($assemblyInfo.FullName)"
    ...
    foreach ($line in $content) {
        if ($line -match $regex) {
            ...
            Write-Host "? Mise � jour $($matches[1]): $($matches[2]) ? $newVersion"
            $newContent += $newline
            $updatedAsm = $true
        } else {
            $newContent += $line
    ...
    if ($updatedAsm) {
        ...
        Write-Host "?? Version mise � jour dans AssemblyInfo.cs"
    } elseif (-not $updatedCsproj) {
        Write-Host "?? Aucune version trouv�e � mettre � jour."
    }
}
...
function Release-FractalC2 {
    ...
    # Fonction pour cr�er un zip par cible
    function Zip-Target {
        param (
            [string]$targetName,
            [string]$sourceDir
    ...
    if ($Target -in @("All","Agent")) {
        ...
        Write-Host "Mise � jour de la version de l'Agent Linux..."
		...
		Write-Host "Mise � jour de la version de l'Agent..."
    ...
    if ($Target -in @("All","TeamServer")) {
        Write-Host "Mise � jour de la version de TeamServer..."
    ...
    if ($Target -in @("All","Commander")) {
        Write-Host "Mise � jour de la version de Commander..."
    ...
    if ($Target -in @("All","WebCommander")) {
        Write-Host "Mise � jour de la version de WebCommander..."
    ...
    Write-Host "Build termin� pour $Target."
}
